...
behavioral_findings <- tribble(
  ~Language, ~Studies, ~N, 
  ~NSYN_Attraction, ~SYN_Attraction, ~Syncretism_Effect, 
  ~Key_Finding,
  
  "English", 
  "Nicol et al. (production, NSYN); Hammerly (speeded, SYN)", 
  NA,
  "2% error increase (4%→6% for pl attractor)", 
  "20% error increase (20%→40% for pl attractor)",
  "Syncretism amplifies attraction 10x",
  "Syncretic conditions show massive attraction effect compared to non-syncretic baseline",
  
  "Turkish", 
  "Turk & Logacev 2024 (ACC, NSYN); Lago et al. 2019 (GEN, SYN)", 
  NA,
  "10% error increase (14%→24% for pl attractor)", 
  "~10% error increase (similar to NSYN)",
  "No modulation",
  "Syncretic genitive case shows similar attraction to non-syncretic accusative",
  
  "German", 
  "Hartsuiker et al. 2003", 
  24,
  "1.8% error increase (4.1%→5.9% for pl attractor, unambiguous)",
  "8.6% error increase (0.9%→9.5% for pl attractor, ambiguous)",
  "Syncretism amplifies attraction ~5x",
  "Case ambiguity (syncretism) substantially amplifies attraction effects",
  
  "Russian", 
  "Slioussar (JML)", 
  48,
  "18.6% error increase (4.8%→23.4% for pl attractor in ACC)",
  "Reversed: 14.5% error for syncretic sg.GEN vs 9.5% for pl.GEN",
  "Syncretism creates reversed pattern",
  "Syncretic sg.GEN (=NOM.PL morphology) triggers more errors than true plurals; attraction follows morphological form"
)

behavioral_findings %>%
  kable(
    caption = "Summary of Human Agreement Attraction Studies by Language",
    col.names = c("Language", "Studies", "N", "Non-Syncretic Attraction", "Syncretic Attraction", "Syncretism Effect", "Key Finding")
  ) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
  column_spec(4, width = "12em") %>%
  column_spec(5, width = "12em") %>%
  column_spec(7, width = "28em")
```
...
# Source: Wagers et al. (2009), Hammerly (speeded), Nicol et al. (production)
# Note: English subject-verb agreement is syncretic (no case marking on nouns)
# Nicol et al. tested possessive constructions which ARE marked (non-syncretic)
english_behavioral <- tribble(
  ~Condition, ~Head_Num, ~Attr_Num, ~Verb_Num, ~is_syn, ~Mean, ~SE, ~Source,
  
  # Nicol et al. (production) - NON-SYNCRETIC (possessive marking: elf's vs elves')
  "SG-SG", "sg", "sg", "sg", "no", 96, NA, "Nicol et al. (production) (NSYN)",
  "SG-PL", "sg", "pl", "sg", "no", 94, NA, "Nicol et al. (production) (NSYN)",
  
  # Wagers et al. (2009) - SYNCRETIC - Grammatical conditions
  "SG-SG-SG", "sg", "sg", "sg", "yes", 93, 3.0, "Wagers et al. (2009) (SYN)",
  "SG-PL-SG", "sg", "pl", "sg", "yes", 91, 3.7, "Wagers et al. (2009) (SYN)",
  # Wagers et al. (2009) - SYNCRETIC - Ungrammatical conditions
  "SG-SG-PL", "sg", "sg", "pl", "yes", 25, 7.0, "Wagers et al. (2009) (SYN)",
  "SG-PL-PL", "sg", "pl", "pl", "yes", 55, 6.0, "Wagers et al. (2009) (SYN)",
  
  # Hammerly (speeded acceptability) - SYNCRETIC
  "GRAM-SG", "sg", "pl", "sg", "yes", 89, 2.0, "Hammerly (speeded) (SYN)",
  "GRAM-PL", "pl", "sg", "pl", "yes", 86, 2.0, "Hammerly (speeded) (SYN)",
  "UNGRAM-SG", "pl", "sg", "sg", "yes", 80, 3.0, "Hammerly (speeded) (SYN)",
  "UNGRAM-PL", "sg", "pl", "pl", "yes", 60, 4.0, "Hammerly (speeded) (SYN)"
) %>%
  mutate(
    Language = "English",
...
english_behavioral %>%
  select(Condition, Grammaticality, Match, is_syn, Mean, SE, Source) %>%
  kable(
    caption = "English Behavioral Data: Accuracy (% correct) - Syncretic vs Non-Syncretic",
    digits = 1
  ) %>%
...
turkish_behavioral <- tribble(
  ...
  "SYN-SG-SG-SG", "sg", "sg", "sg", "yes", 92, 1.0, "Lago et al. (2019) (SYN)",
  "SYN-SG-PL-SG", "sg", "pl", "sg", "yes", 91, 1.0, "Lago et al. (2019) (SYN)",
  "SYN-SG-SG-PL", "sg", "sg", "pl", "yes", 86, 1.0, "Lago et al. (2019) (SYN)",
  "SYN-SG-PL-PL", "sg", "pl", "pl", "yes", 76, 1.0, "Lago et al. (2019) (SYN)"
) %>%
  mutate(
    Language = "Turkish",
    Match = ifelse(Head_Num == Attr_Num, "Match", "Mismatch"),
...
# Function to safely load CSV with optional language tag
safe_read <- function(path, name, add_lg = NULL) {
  if (file.exists(path)) {
    df <- read_csv(path, show_col_types = FALSE)
    # Add language column if not present and add_lg is specified
    if (!is.null(add_lg) && !"lg" %in% names(df)) {
      df <- df %>% mutate(lg = add_lg)
    }
...
# Standardize English attention/surprisal columns to match Turkish/Russian format
standardize_english <- function(df) {
  if (nrow(df) == 0 || !"NP_Number" %in% names(df)) return(df)

  df %>%
    mutate(
      lg = "english",
      # NP_Number is the attractor number
      attn = case_when(
        tolower(NP_Number) == "singular" ~ "sg",
        tolower(NP_Number) == "plural" ~ "pl",
        TRUE ~ tolower(NP_Number)
      ),
      # Auxiliary encodes verb number
      verbn = case_when(
        tolower(Auxiliary) == "is" ~ "sg",
        tolower(Auxiliary) == "are" ~ "pl",
        tolower(Auxiliary) == "was" ~ "sg",
        tolower(Auxiliary) == "were" ~ "pl",
        TRUE ~ tolower(Auxiliary)
      ),
      # source_file encodes syncretism
      syn = ifelse(source_file == "syn", 1, 0),
      # Head is always singular in these stimuli
      headn = "sg",
      # Standardize item column name
      item = Item
    ) %>%
    select(-Item, -NP_Number, -Auxiliary, -source_file)
}

# Standardize German attention/surprisal columns to match Turkish/Russian format
standardize_german <- function(df) {
  if (nrow(df) == 0 || !"attractor" %in% names(df)) return(df)

  df %>%
    mutate(
      lg = "german",
      # attractor column is already sg/pl
      attn = attractor,
      # verb column is already sg/pl
      verbn = verb,
      # case "amb" means syncretic (ambiguous)
      syn = ifelse(case == "amb", 1, 0),
      # Head is always singular in these stimuli
      headn = "sg"
    ) %>%
    select(-attractor, -verb, -case)
}
...
attention_results <- bind_rows(
  ...
  safe_read(result_paths$attention_english, "attention_english") %>% standardize_english(),
  safe_read(result_paths$attention_german, "attention_german") %>% standardize_german(),
...
surprisal_results <- bind_rows(
  ...
  safe_read(result_paths$surprisal_english, "surprisal_english") %>% standardize_english(),
  safe_read(result_paths$surprisal_german, "surprisal_german") %>% standardize_german(),
...
#| fig-height: 6
  ...
  multihead_results %>%
    ...
    geom_tile() +
    scale_fill_viridis_c(option = "plasma", limits = c(0, 0.5)) +
    facet_grid(direction ~ language) +
    labs(
...
#| fig-height: 6
  ...
  att_plot_data <- attention_results %>%
    ...
    att_plot_data <- att_plot_data %>%
      mutate(
        Match = ifelse(headn== attn, "Match", "Mismatch"),
        Grammaticality = ifelse(headn == verbn, "Grammatical", "Ungrammatical"),
...
# This assumes both datasets are available and properly formatted
  ...
  # Combine all four languages' behavioral data
  human_attraction_effects <- bind_rows(
    english_behavioral,
    turkish_behavioral,
    german_behavioral,
    russian_behavioral
    ...
    filter(!is.na(Mean)) %>%
    group_by(Language, Grammaticality, Match, Syncretism) %>%
    summarise(mean_val = mean(Mean), .groups = "drop") %>%
    pivot_wider(names_from = Match, values_from = mean_val) %>%
    mutate(
      attraction_effect = abs(Match - Mismatch),  # For accuracy: Match > Mismatch means attraction
      source = "Human",
      measure = "Accuracy (%)"
    )
  ...
  p2 <- human_attraction_effects %>%
    ggplot(aes(x = Language, y = attraction_effect, fill = Syncretism)) +
    geom_col(position = "dodge") +
    geom_hline(yintercept = 0, linetype = "dashed") +
  ...
  ## TODO: repeat the production results in both grammatical and ungrammatical. 
