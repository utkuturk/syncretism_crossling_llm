...
BERT_CHUNK_PATTERN = os.path.join(DATA_DIR, "turkish_bert_attention_nsubj_root_chunk_*.csv")
GPT2_CHUNK_PATTERN = os.path.join(DATA_DIR, "gpt2_turkish_attention_nsubj_root_chunk_*.csv")
...
def load_all_chunks(chunk_pattern: str, model_name: str) -> pd.DataFrame:
    ...
    if not files:
        raise FileNotFoundError(f"[{model_name}] No files found for pattern: {chunk_pattern}")
...
def main():
    ...
    # If you **also** want the combined raw attention file, uncomment:
    # gpt2_full_csv = os.path.join(RESULTS_DIR, "gpt2_turkish_attention_nsubj_root_full.csv")
    # gpt2_df.to_csv(gpt2_full_csv, index=False)
    # print(f"[GPT-2] Combined attention CSV saved to: {gpt2_full_csv}")
    ...
    # Same here if you want a single combined raw CSV:
    # bert_full_csv = os.path.join(RESULTS_DIR, "turkish_bert_attention_nsubj_root_full.csv")
    # bert_df.to_csv(bert_full_csv, index=False)
    # print(f"[BERT] Combined attention CSV saved to: {bert_full_csv}")
