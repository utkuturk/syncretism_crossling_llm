...
def compute_bert_attention(df: pd.DataFrame, log_every: int = 500) -> pd.DataFrame:
    ...
    for i, (_, row) in enumerate(df.iterrows(), start=1):
        if pd.isna(row.get("sentence")) or pd.isna(row.get("nsubj_first")) or pd.isna(row.get("root_first")):
            continue
        ...
        if seq_len > max_len:
            skipped_too_long += 1
            if i % log_every == 0:
                print(f"[BERT] {i}/{total_rows} | skipped long: {skipped_too_long}...", flush=True)
            continue
        ...
        if nsubj_span is None or root_span is None:
            skipped_no_span += 1
            if i % log_every == 0:
                print(f"[BERT] {i}/{total_rows} | skipped no span: {skipped_no_span}...", flush=True)
            continue
        ...
        if root_idx < nsubj_idx:
            skipped_order += 1
            if i % log_every == 0:
                print(f"[BERT] {i}/{total_rows} | skipped order: {skipped_order}...", flush=True)
            continue
    ...
    print(f"[BERT] Done chunk. Kept: {len(results)}")
...
def compute_gpt2_attention(df: pd.DataFrame, log_every: int = 500) -> pd.DataFrame:
    """
    For each sentence, find the strongest attention from root to nsubj
    across all layers/heads in GPT-2.
    """
    ...
    for i, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        if max_len is not None and seq_len > max_len:
            skipped_too_long += 1
            if i % log_every == 0:
                print(f"[GPT-2] {i}/{total_rows} | skipped long: {skipped_too_long}...", flush=True)
            continue
        ...
        if root_span is None or nsubj_span is None:
            skipped_no_span += 1
            if i % log_every == 0:
                print(f"[GPT-2] {i}/{total_rows} | skipped no span: {skipped_no_span}...", flush=True)
            continue
        ...
        if nsubj_idx >= root_idx:
            skipped_order += 1
            if i % log_every == 0:
                print(f"[GPT-2] {i}/{total_rows} | skipped order: {skipped_order}...", flush=True)
            continue
    ...
    print(f"[GPT-2] Done chunk. Kept: {len(results)}")
    return pd.DataFrame(results)
